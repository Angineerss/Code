...
def _labeled_frame() -> pd.DataFrame:
    ...
    # bar spans keep seconds/bar ~ 1 hour so 1τ=30-bar purge/embargo ≈ 30h.
    return pd.DataFrame(
        {
...
def test_filter_with_purge_and_embargo_defaults():
    ...
    # Policy A: Purge + Embargo = 1τ (None → follow vertical_bars=30).
    assert config.purge_bars is None
    assert config.embargo_bars is None
    assert config.resolved_purge_bars() == config.vertical_bars == 30
    assert config.resolved_embargo_bars() == 30
