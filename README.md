...
초록 : 바이낸스 BTCUSDT의 틱 데이터를 Binance Vision에서 수집해 달러가 하루 평균 거래대금/650이라는 임계값을 넘는 경우 바를 생성하여, 하루의 EWMA로 이 임계값을 갱신함. 이 달러 바의 불균형 정도가 임계점을 넘으면 그 방향을 매매 방향으로 설정하는 1차 모델과 이 방향이 충분히 강할 때 체결하는 2차 모델로 구성해, 바가 익절 벽을 먼저 건드린 경우 1을 부여하고 그 외에는 0을 부여하여 매매 프로그램을 개선함. 이 모델을 확정한 후 OOS로 백테스팅해서 모델을 완성 
...
코드 주석: **`[선정]`** = 이유를 단 것. **`[임시값]`** = 아직 이유 없음.

**이름:** 문서 `daily_T$` · 말 **어제 조각** · 식 `D = 어제 대금 / 650`. 매일 UTC일마다 다시 계산하는 그날 T$ 기준이다. 파이프라인 전체의 단 한 번 초기값이 아니다. 클립 식은 `[0.5D, 2D]`.

## 실행

테스트는 합성 틱만 쓰고 **네트워크가 필요 없습니다.** 하루 파이프라인·아카이브는 Vision/`api.binance.com`이 필요합니다. `data/` 는 git에 넣지 않습니다.
...
Windows 로컬 가상환경이면 `python` 대신 `.\.venv\Scripts\python.exe` 를 씁니다.

학습 구간(워밍업 바 → IS 라벨). OOS는 넣지 않습니다.

```bash
python scripts/download_aggtrades_archive.py --data-dir data/aggtrades   # ~58GB, 스모크에 쓰지 말 것
python scripts/run_learning_range.py --skip-existing
python scripts/run_learning_range.py --bar-type dollar_imbalance --skip-existing   # 대조군, 폴더 분리
...
OOS(`2025-01-01` 이후)는 `--allow-oos` 없이 거부됩니다. 확정 후 백테스트만.

산출물 (`data/`):

- `aggtrades/monthly/{SYMBOL}-aggTrades-YYYY-MM.zip` — 상장 이후 월간 아카이브
- `aggtrades/daily/{SYMBOL}-aggTrades-YYYY-MM-DD.zip` — 미마감 월 일간
- `{SYMBOL}_{day}_bars.csv` / `_labels.csv` / `_ewma_state.json` / `_summary.json`

## 저장소

| 경로 | 역할 |
| --- | --- |
| `src/` | 하루 파이프라인. 설정은 `src/config.py` |
| `scripts/` | 구간 실행, divisor·τ·pt/sl 비교, 아카이브 다운로드 |
| `tests/` | 합성 틱. `python -m pytest` |
| `results/` | 스크리닝 JSON (예: `divisor_bar_screen.json`) |
| `data/` | 틱·바·라벨. git 무시 |
...
| # | 항목 | 이유 |
| --- | --- | --- |
| 1 | `flow_strength` | 달러 바로 자르니 세기도 그 바 대금으로 나눔 (`\|θ\|/바대금`) |
...
| 20 | `require_strong_imbalance=False` | AFML: primary는 recall을 키운다 |
...
| 27* | purge·embargo 사용 | OOS가 학습에 안 들어가게. **길이는 임시값(1τ)** |
...
**임시값** (이유 미작성): `sigma` (피처·벽 넓이 같은 하나, 길이 50), T$ EWMA 길이 50, `pt`/`sl`/`τ`, RF 크기(200그루·깊이 6·잎 10), 거래소 외 UTC·세션 필터, purge·embargo **길이** 1τ. `max_ticks`·`init_T`·대조군 틱 배수(2.5)의 **숫자**도 여기.
